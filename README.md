...
**Пример настроенной сцены** (загрузка моделей и CSV, показ групп, материалы, маркеры) — в файле `example-scene.js`. Запуск: откройте `index-example.html` в браузере (через локальный сервер или двойной клик).
